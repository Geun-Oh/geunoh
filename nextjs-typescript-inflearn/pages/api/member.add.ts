...
export default async function handler(
  ...
  try {
    const addResult = await FirebaseAdmin.getInstance().Firebase.collection("members").doc(uid).set({
        uid,
        email,
    ...
    await FirebaseAdmin.getInstance().Firebase.collection("screen_names").doc(screenName).set({
      uid,
      email,
      displayName: displayName ?? "",
      photoURL: photoURL ?? "",
